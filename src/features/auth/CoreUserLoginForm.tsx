...
import { useCoreLoginMutation } from "../../store/api";
...
export const CoreUserLoginForm: React.FC = () => {
  const [username, setUsername] = useState("");
  const [password, setPassword] = useState("");
  const [login, { isLoading }] = useCoreLoginMutation();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.token) {
        localStorage.setItem("coreUserToken", response.token);
        localStorage.setItem("activeUserType", "core");
        setFormFeedback({
          type: "success",
      ...
      console.error("Core User login failed:", err);
      ...
      <div className="p-8 bg-white shadow-xl rounded-lg w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <SubmitButton
            ...
            className="bg-blue-600 hover:bg-blue-700 focus:ring-blue-500 mt-8"
          />
        </form>
      </div>
    </div>
